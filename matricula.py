...
def abrir_login(event):
    ...
    exec(open("entrar.py").read())  # Abre a janela de login
...
# Título "Matrícula"
titulo_label = tk.Label(register_box, text="Matrícula", font=("Arial", 16, "bold"), fg="#FF6E14", bg="white")
titulo_label.pack()
...
tk.Label(register_frame, text="Nome completo:", font=("Arial", 12), bg="white").grid(row=0, column=0, pady=10, padx=10, sticky='w')
name_entry = tk.Entry(register_frame, width=30, bd=2)
name_entry.grid(row=0, column=1, pady=10, sticky='w')

# Caixa de email
tk.Label(register_frame, text="Email:", font=("Arial", 12), bg="white").grid(row=1, column=0, pady=10, padx=10, sticky='w')
email_entry = tk.Entry(register_frame, width=30, bd=2)
...
education_label = tk.Label(register_frame, text="Selecione sua escolaridade:", font=("Arial", 12), bg="white")
education_label.grid(row=3, column=0, pady=10, padx=10, sticky='w')

education_var = tk.StringVar()
education_menu = tk.OptionMenu(register_frame, education_var, "Selecione sua escolaridade/área de atuação")
education_menu.grid(row=3, column=1, pady=10, sticky='w')

# Caixa de senha
tk.Label(register_frame, text="Senha:", font=("Arial", 12), bg="white").grid(row=4, column=0, pady=10, padx=10, sticky='w')
password_entry = tk.Entry(register_frame, width=30, bd=2, show="*")
password_entry.grid(row=4, column=1, pady=10, sticky='w')

# Caixa de confirmação de senha
tk.Label(register_frame, text="Confirmar senha:", font=("Arial", 12), bg="white").grid(row=5, column=0, pady=10, padx=10, sticky='w')
confirm_password_entry = tk.Entry(register_frame, width=30, bd=2, show="*")
confirm_password_entry.grid(row=5, column=1, pady=10, sticky='w')

# Botão de matricular
register_button = tk.Button(register_frame, text="Matricular", font=("Arial", 12), bg="#FF6E14", fg="white", width=20, command=register)
register_button.grid(row=6, columnspan=2, pady=20)

# Link para login
login_label = tk.Label(root, text="Já tenho matrícula", fg="#FF6E14", bg="white", cursor="hand2", font=("Arial", 12, "underline"))
